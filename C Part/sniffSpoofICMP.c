...
got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	const struct icmpheader *oldICMP;
	...
	switch(ip->ip_p) {
		...
		case IPPROTO_ICMP:
			...
			//might use function: injectPacket(); , but we want the id and Seq of the ICMP req to be correct with the reply.
			/*************/
    ...
    ********************************************************/
   struct icmpheader *icmpOLD = (struct icmpheader *) 
                             (packet + sizeof(struct ipheader) + SIZE_ETHERNET);
   struct icmpheader *icmp = (struct icmpheader *) 
                             (buffer + sizeof(struct ipheader));
   icmp->icmp_type = 0; //ICMP Type: 8 is request, 0 is reply.
   icmp->icmp_id = icmpOLD->icmp_id;
   icmp->icmp_seq = icmpOLD->icmp_seq;
   ...
   ipNew->iph_ihl = 5;
   ipNew->iph_ttl = 20;
   ipNew->iph_sourceip.s_addr = inet_addr(inet_ntoa(ip->ip_dst));
   ipNew->iph_destip.s_addr =  inet_addr(inet_ntoa(ip->ip_src));
...
int main(int argc, char **argv)
{
	...
	char filter_exp[] = "icmp"; // or (src host 10.0.2.20 and dst host 10.0.2.4)";		/* filter expression [3] */
